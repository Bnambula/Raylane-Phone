...
export default function AdminDashboard() {
  const { state, store:st, unreadCount, pendingTrips, pendingApps, totalRevenue, totalCommission } = useAdminStore()
  ...
  const Sidebar = () => (
    <div style={{ width:206, background:'#0a0f1e', flexShrink:0, display:'flex', flexDirection:'column', overflowY:'auto' }}>
      ...
      <div style={{ padding:'10px 12px', borderTop:'1px solid rgba(255,255,255,.07)', display:'flex', flexDirection:'column', gap:6 }}>
        <button onClick={()=>{store.resetToInitial();toast('Demo reset','success')}} style={{ width:'100%', padding:'6px', borderRadius:7, background:'rgba(255,100,100,.12)', color:'rgba(255,150,150,.8)', border:'none', cursor:'pointer', ...P, fontWeight:600, fontSize:9 }}>Reset Demo</button>
      ...
      <div style={{ flex:1, overflowY:'auto', padding:22 }}>
        ...
        {active==='alerts' && (
          <Card>
            ...
            {state.notifications.admin.length===0 ? <EmptyState icon="[OK]" title="All caught up!"/> : state.notifications.admin.map((n,i)=>(
              <div key={n.id} style={{ display:'flex', alignItems:'flex-start', gap:12, padding:'12px 0', borderBottom:i<state.notifications.admin.length-1?'1px solid #E2E8F0':'', background:!n.read?'#f8faff':'' }}>
