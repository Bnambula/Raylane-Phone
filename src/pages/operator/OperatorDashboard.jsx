  ...
  return (
    <div style={{ display:'flex', height:'100vh', overflow:'hidden', background:'#F5F7FA', paddingTop:'var(--nav-h)' }}>
      ...
      <Modal open={!!pModal} onClose={() => setPModal(null)} title={pModal==='add'?'Add Invoice':'Edit Invoice'}>
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12 }}>
          <div><label style={lS}>Type</label>
            <select defaultValue={typeof pModal==='object'?pModal.type:'Utility'} id="p-type-rlx" style={iS}>
              {['Utility','Rent','Washing','Internet','Insurance','Permit','Other'].map(t=><option key={t}>{t}</option>)}
            </select>
          </div>
          <div><label style={lS}>Vendor</label>
            <select defaultValue={typeof pModal==='object'?pModal.vendor:''} id="p-vendor-rlx" style={iS}>
              <option value="">Select...</option>
              {demoVendors.map(v=><option key={v.id}>{v.name}</option>)}
            </select>
          </div>
          <div style={{ gridColumn:'1/-1' }}><label style={lS}>Description</label>
            <input defaultValue={typeof pModal==='object'?pModal.desc:''} id="p-desc-rlx" style={iS}/>
          </div>
          <div><label style={lS}>Amount (UGX)</label><input type="number" defaultValue={typeof pModal==='object'?pModal.amount:''} id="p-amount-rlx" style={iS}/></div>
          <div><label style={lS}>Due Date</label><input type="date" defaultValue={typeof pModal==='object'?pModal.due:''} id="p-due-rlx" style={iS}/></div>
        </div>
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:10, marginTop:16 }}>
          <Btn variant="ghost" full onClick={() => setPModal(null)}>Cancel</Btn>
          <Btn variant="blue" full onClick={() => {
            const amount = parseInt(document.getElementById('p-amount-rlx')?.value||0)
            const desc   = document.getElementById('p-desc-rlx')?.value||''
            const type   = document.getElementById('p-type-rlx')?.value||'Utility'
            const vendor = document.getElementById('p-vendor-rlx')?.value||''
            const due    = document.getElementById('p-due-rlx')?.value||''
            if (pModal==='add') {
              setOpPayments(p=>[...p,{id:'PMT'+Date.now(),type,vendor,desc,amount,due,status:'UNPAID'}])
              toast('Invoice added','success')
            } else {
              setOpPayments(p=>p.map(x=>x.id===pModal.id?{...x,amount,desc,type,vendor,due}:x))
