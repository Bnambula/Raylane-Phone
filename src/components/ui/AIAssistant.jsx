...
const QUICK = [
  { icon:'[CLOCK]', q:'Best time to travel?', a:'Weekday mornings (Tue-Thu, 6-9 AM) are least busy. Avoid Friday 4-8 PM -- peak traffic out of Kampala.' },
  { icon:'[CASH]', q:'Cheapest route today?', a:'Kampala -> Jinja from UGX 12,000. Kampala -> Entebbe from UGX 8,000. Prices rise ~15% after 2 PM.' },
  { icon:'[BOX]', q:'How to send a parcel?', a:'Go to Parcels -> choose size (Envelope from UGX 5K to Heavy Cargo UGX 30K) -> pay via MoMo -> drop at any Raylane hub.' },
  { icon:'[BUS]', q:'How do I join as operator?', a:'Click "Join Now" or visit /partner. Fill the form -- our team reviews within 24-48 hrs and sets up your dashboard.' },
]

const REPLIES = {
  default: 'I can help with routes, booking, parcels, and payments. What would you like to know?',
  cheap:   '[CASH] Kampala -> Jinja from UGX 12,000. -> Entebbe from UGX 8,000. Prices rise after 2 PM -- book early!',
  parcel:  '[BOX] Send from UGX 5,000 (envelope) to UGX 30,000 (heavy cargo). All parcels get real-time tracking!',
  book:    '[TKT] Search route -> choose vehicle type -> select seat -> pay via MTN MoMo or Airtel. Done in under 2 minutes!',
  time:    '[CLOCK] Best: Tue-Thu 6-9 AM. Avoid Friday evenings. Weekend mornings also good for long routes.',
  nairobi: '[GLOBE] Kampala->Nairobi: UGX 60,000 . ~8 hrs. Kampala->Kigali: UGX 55,000 . ~9 hrs. Kampala->Juba: UGX 60,000 . ~12 hrs.',
  pay:     '[PHONE] We accept MTN Mobile Money & Airtel Money. After selecting your seat, a prompt is sent to your phone to confirm payment.',
  operator:'[BUS] Join via /partner. Free onboarding. You get a full dashboard to manage routes, bookings, and passengers.',
      ...
      {open && (
        <div style={{ position:'fixed', bottom:76, right:16, zIndex:849, width:'min(340px,calc(100vw - 32px))', background:'var(--white)', borderRadius:20, boxShadow:'0 16px 48px rgba(0,0,0,0.2)', border:'1px solid var(--gray-mid)', overflow:'hidden', display:'flex', flexDirection:'column', animation:'fadeIn .25s ease', maxHeight:'70vh' }}>
          ...
          <div style={{ background:'var(--blue)', padding:'14px 18px', color:'var(--white)', display:'flex', alignItems:'center', gap:10 }}>
            ...
            <div>
              <div style={{ fontFamily:"'Poppins',sans-serif", fontWeight:800, fontSize:13 }}>Raylane AI</div>
          ...
          <div style={{ padding:'6px 12px', display:'flex', gap:5, overflowX:'auto', borderTop:'1px solid var(--gray-mid)' }}>
            {QUICK.map(q => (
              <button key={q.q} onClick={() => send(q.q)} style={{ background:'var(--gray-light)', border:'1px solid var(--gray-mid)', borderRadius:16, padding:'5px 10px', fontSize:10, fontFamily:"'Poppins',sans-serif", fontWeight:600, cursor:'pointer', whiteSpace:'nowrap', flexShrink:0 }}>
